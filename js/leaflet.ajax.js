L.GeoJSON.AJAX=L.GeoJSON.extend({
    ...
    addUrl: function (url) {
        ...
        if(this.ajaxParams.dataType.toLowerCase()==="json"){
          L.Util.ajax(url, function(d){var data = _this.ajaxParams.middleware(d);_this._cache=_this._cache.concat(data.features);_this.addData(data.features);_this.fire("dataLoaded");}); 
        }else if(this.ajaxParams.dataType.toLowerCase()==="jsonp"){
            L.Util.jsonp(url, function(d){var data = _this.ajaxParams.middleware(d);_this._cache=_this._cache.concat(data.features);_this.addData(data.features);_this.fire("dataLoaded");}, _this.ajaxParams.callbackParam);
        }
    },
